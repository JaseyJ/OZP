...
ngram_size = 2 #determine window size of n-gram
...
while i < arr.size:  #while loop to n-gramify each entry separatly and append corresponding score to front of array
    ...
    if lengte >= ngram_size: #apply standard procedure when length of input is greater than or equal to window size
...
c = Counter(vocab)
selected_vocab = Counter({k: c for k, c in c.items() if c >= 30}) #Count amount of times n-gram appears, add to selected vocab if higher than given value
